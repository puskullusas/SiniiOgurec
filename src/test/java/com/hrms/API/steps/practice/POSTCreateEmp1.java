package com.hrms.API.steps.practice;
	...
	public class POSTCreateEmp1 {
		...
		public void user_calls_createEmployee_API_to_verify_that_an_employee_was_created() {
			...
			request = given().header("Content-Type", "application/json").header("Authorization",
					SyntaxAPIAuthenticationSteps.Token);
												//System.getProperty("user.dir")+ "/src/test/resources/JSONFiles/generateToken.json"
			request.body(CommonMethods.readJson(APIConstants.CREATE_EMP_JSON));
		...
		response.then().body("Employee[0].emp_firstname", equalTo("Master"));
